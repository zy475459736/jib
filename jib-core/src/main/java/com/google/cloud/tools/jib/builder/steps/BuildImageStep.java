...
import com.google.cloud.tools.jib.image.LayerHistory;
...
class BuildImageStep
  ...
  private Image<CachedLayer> afterCachedLayersSteps()
    ...
    try (Timer ignored = new Timer(buildConfiguration.getBuildLogger(), DESCRIPTION)) {
      ...
      // Builds the image history.
      LayerHistory.Builder layerHistoryBuilder = LayerHistory.builder(baseImage.getHistory());
      if (containerConfiguration != null) {
        layerHistoryBuilder
            .getHistoryEntryBuilder()
            .setCreationTimestamp(containerConfiguration.getCreationTime());
      }
      // Adds history elements for base image layers that don't have one yet.
      layerHistoryBuilder.autofillTo(baseImageLayers.size());
      // Adds history elements for application layers.
      layerHistoryBuilder.fillTo(
          baseImageLayers.size() + buildAndCacheApplicationLayerSteps.size(),
          buildConfiguration.getToolName());
      imageBuilder.setHistory(layerHistoryBuilder.build().getHistoryEntries());
